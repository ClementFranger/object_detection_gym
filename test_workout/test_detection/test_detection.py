...
class TestDetection(TestTensorflow):
    ...
    def test_(self):
        assert isinstance(Detection.instance, Detection)
        assert Detection.instance.name == 'saved_model'
        assert os.path.isdir(Detection.instance.source)
        assert os.path.isdir(Detection.instance.path)

    def test_model(self):
        assert isinstance(Detection.instance.model, Model)
        assert os.path.isdir(Detection.instance.model.path)

    def test_detect_image(self):

        Detection.instance.detect_image(labels=Data.instance.labels_pbtxt, image=r'C:\Users\Minifranger\Documents\python_scripts\workout\workout\overwatch\data\images\1614284494123.jpg')
